...
function footnote(node) {
    ...
    return self.visitors.footnoteReference.call(self, {
...
function link(node) {
    return h(this, node, 'a', {
        'href': util.normalizeURI(node.url || ''),
        'title': node.title
    }, this.all(node));
}
...
function linkReference(node) {
    ...
    return failsafe(node, def, self) || h(self, node, 'a', {
        'href': util.normalizeURI(def.url || ''),
        'title': def.title
    }, self.all(node));
}
...
function image(node) {
    return h(this, node, 'img', {
        'src': util.normalizeURI(node.url || ''),
        'alt': node.alt || '',
        'title': node.title
    });
}
...
compilers.all = all;
compilers.generateFootnotes = generateFootnotes;
compilers.getDefinition = getDefinition;
